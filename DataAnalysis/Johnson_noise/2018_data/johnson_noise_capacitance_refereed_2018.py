'''
This file is to change the graphical plot according to the referee's suggestions.
'''
...
def fit_johnson_model(params, x, gain_x, gain_y):
    ...
    B = params['Bf'].value
    C = params['Cf'].value
    delta_f = np.ones_like(x)
    for i in range(len(x)):
        new_gain_y = gain_y/(1+(2*np.pi*gain_x*x[i]*C)**2)
        delta_f[i] = np.sum(new_gain_y*interval)#/area
    #print "x is " + str(x)
    #print "new_R is " + str(new_R)
    output = A*(x*delta_f)**B
    #print x
    return output
'''
define how to compare data to the function
...
print "k is " + str(result.params['Af']/(4*(23.8+273.15)))
...
pyplot.plot(x_plot,fit_johnson_model(result.params,x_plot, gain_x, gain_y),linewidth = 2.0, color = 'green')
...
pyplot.errorbar(x,y2,yerr, linestyle='None',markersize = 6.0,fmt='x',color='black')
pyplot.plot(x_plot,fit_johnson_model(result.params,x_plot, gain_x, gain_y),linewidth = 2.0, color = 'blue')
